...
The failure this file is built around. A store comparison does not turn on is
not "the real one is better". It is the case where the SCORE is identical and
the RETRIEVED DOCUMENTS are not: a project that reported only the score would
publish "the stand-in cost us nothing" while its two stores agreed on almost
nothing. That case cannot be produced on demand from a real Elasticsearch, so
...
def test_two_stores_that_both_return_nothing_have_not_agreed():
    ...
    The relational leg was compared on the eleven document-bearing queries, the
    ones it is not for, where both implementations correctly return nothing.
    Eleven empty-vs-empty pairs were counted as eleven agreements, and the
    headline announced that the stand-in cost nothing. The comparison had
