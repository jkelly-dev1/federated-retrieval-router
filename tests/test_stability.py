...
def test_never_losing_is_not_the_same_finding_as_straddling():
    ...
    sample cannot order two routers it has just ordered. The same imprecision
    this comparison exists to remove, moved one step over.
    """
    queries = _queries()
    baseline = summarize_runs(
        "heuristic",
...
def test_two_identical_routers_are_clear_on_neither_axis():
    """The degenerate case. Neither axis separates, so nothing survives, but
    note that this fixture CANNOT tell `and` from `or` in the verdict, because
    both conjuncts are False. See the test below for the one that can."""
    queries = _queries()
    baseline = summarize_runs("heuristic", queries, _perfect(queries, 3), PRODUCTION_MIX)
    challenger = summarize_runs(
...
def test_a_cheaper_router_that_is_not_more_correct_does_not_survive():
    """Both axes, not the better of the two, and this fixture proves it.

    The fixture has to instantiate the condition, and an assertion about
    `and` is worth nothing unless exactly one conjunct is true. Two routers
    built from the same fixture are clear on neither axis, so `and` and `or`
    agree and the operator is unpinned, that degenerate case is the test
    above, kept and labeled.

    Here the challenger is genuinely CHEAPER (one backend for everything,
    fan-out 1.0 against the baseline's 2.0) and genuinely LESS CORRECT (right
    ...
    """
    queries = _queries()
    # Baseline: always right, and always pays two backends to be right.
    two_each = [_run(queries, [q.required | GRAPH for q in queries]) for _ in range(3)]
    baseline = summarize_runs("heuristic", queries, two_each, PRODUCTION_MIX)
    # Challenger: one backend for everything. Cheaper, and wrong wherever the
    # answer was not in that backend.
    one_each = [_run(queries, [VECTOR for _ in queries]) for _ in range(3)]
    challenger = summarize_runs("llm-anthropic", queries, one_each, PRODUCTION_MIX)

    verdict = ranking_survives(challenger, baseline)
    assert verdict.fan_out_clear, (
        "the fixture did not build a cheaper challenger, so this test cannot "
        "distinguish `and` from `or` and proves nothing")
    assert not verdict.correctness_clear, (
        "the fixture did not build a less correct challenger")
    assert not verdict.survives, (
        "a cheaper but less correct router survived: the verdict is accepting "
        "one axis instead of requiring both")
