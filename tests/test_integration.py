...
here skips, loudly, with a reason naming what was missing, instead of failing.
A red suite that means "you did not start a container" trains people to ignore
red suites.
...
what scripts/compare_stores.py is for, and pinning it in a test would turn a
measurement into a threshold that fails whenever a store version moves.
