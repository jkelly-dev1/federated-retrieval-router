...
def test_an_adapter_with_no_client_and_no_address_says_which_is_missing():
    ...
    with pytest.raises(ValueError, match="url or an injected client"):
        ElasticsearchBackend(_docs(), client=None, url=None)
...
def test_a_missing_address_is_reported_before_a_missing_driver():
    ...
    for factory in (
        ...
        lambda: ElasticsearchBackend(_docs(), client=None, url=None),
    ):
        with pytest.raises(ValueError) as exc:
            factory()
        assert "not installed" not in str(exc.value), (
            f"a configuration error surfaced as a dependency error: {exc.value}")
...
def test_the_matching_analyzer_mirrors_this_repositorys_own_tokenizer():
    ...
    identical tokenization when the two engines apply it at different points in the pipeline. What
    settles it is running text through the real _analyze endpoint and comparing token lists: see
