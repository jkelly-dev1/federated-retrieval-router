...
def test_an_explicit_variable_beats_the_env_file(monkeypatch, tmp_path):
    ...
    building providers with a key present constructs a live SDK client, and a
    test that needs the optional dependency in order to make its point is the
    exact test that passes locally and fails in CI.
    """
    import os
...
def test_an_invented_backend_name_is_a_parse_failure():
    ...
    router = _router('{"backends": ["elasticsearch"], "why": "x"}')
    decision = router.route("q", "q1")
    assert decision.chosen == frozenset()
    assert decision.query_id == "q1"
    assert router.failures, "an invented name was not recorded as a failure"


def test_a_partly_invented_reply_is_still_recorded_as_a_failure():
    """The case the test above cannot see.

    Its reply names ONLY the invented backend, so `chosen` is empty whether or
    not the ValueError is caught and recorded, and it never inspects
    `failures`. Wrapping the Backend() construction in a bare `except
    ValueError: pass` therefore left the suite green, and a reply that was
    half hallucination scored as a clean decision with a real backend in it
    and nothing marking the run.

    Here the reply names one real store and one invented one. The real one
    must not launder the invented one.
    """
    router = _router('{"backends": ["vector", "elasticsearch"], "why": "x"}')
    decision = router.route("q", "q1")
    assert router.failures, (
        "a reply naming one real and one invented backend was recorded as a "
        "clean decision")
