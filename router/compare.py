...
that cannot be attributed to anything -- which is how "we moved to a real
vector store and quality improved" gets published without evidence.
...
THREE NUMBERS, BECAUSE ONE WOULD HIDE THE INTERESTING CASE.
  ...
  AGREEMENT    on how many queries did the two implementations return the SAME
               documents? This is the number people never report, and it is the
               one that says whether a delta of zero means "the store does not
               matter" or "the stores disagree constantly and it happens to
               come out even".
...
class QueryDelta:
    ...
    def document_scored(self) -> bool:
        ...
        a document ground truth -- aggregate answers appear in no document --
        and it has to be a query this leg is actually required or sufficient
        for. Scoring the relational leg on a paraphrase question measures it on
...
def compare_backend(
    ...
    # perfect consistency. Which queries a
    # given number is defined over is decided in StoreComparison, per number.
    deltas = [
        QueryDelta(
            query_id=q.query_id,
