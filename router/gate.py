...
It fails in both directions, which is the only version worth having. Failing
when routing regresses is obvious. Failing when EVERYTHING PASSES is the part
that matters: if the vector-only baseline stops losing, or the traps stop
trapping, or the corpus stops being big enough to discriminate, then the
evaluation has stopped measuring and every subsequent green run is worthless.
...
most valuable ones here:
  ...
  with measurement on 1 of 11 queries and the whole evaluation was noise. The
  gate now fails if k/N rises above a threshold, because a corpus that stops
...
def main() -> int:
    ...
    for r in results:
        ...
        print(f"  [{flag}] {r.name:{width}s}  {r.detail}")
    failed = [r for r in results if not r.passed]
    print()
    if failed:
        print(f"GATE FAILED ({len(failed)} of {len(results)} checks)")
        return 1
    print(f"GATE PASSED ({len(results)} checks)")
    print("  routing is still measurable, a single store still loses, and the")
    print("  corpus can still tell one competence from another.")
    return 0
