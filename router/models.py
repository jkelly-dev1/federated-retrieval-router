...
class FusedHit:
    ...
    the field that makes "the graph leg earned its cost" a measurable
    statement rather than a claim: a document only the graph leg found has
...
class LabeledQuery:
    ...
    distinction decides every score below. It records which Backend the question
    shape calls for. A paraphrase needs semantic retrieval, an identifier needs
    an inverted index, a traversal needs a graph. It does NOT record which
    backend happens to win on this corpus with this embedder, and on the offline
    stack those two answers differ.
