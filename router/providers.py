...
questions the deterministic stack structurally cannot:
  ...
  on the same two axes: correctness AND fan-out. A model that routes perfectly
...
# and never mentions the corpus, the entity names or the heuristic's tells. A
# model told "config keys look like a.b.c" would be reciting the guard rather
...
class Providers:
    ...
    and worthless question, whether a dict returns what was put into it, so
    the repeat has to pay for its own calls. It is one extra pass over the
    corpus and the cheapest section of the run.
    """
