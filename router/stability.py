...
its spread -- is not a result. This module exists so the capture cannot say
that, and it deliberately measures something more useful than a mean.
...
WHAT n=5 BUYS AND WHAT IT DOES NOT, stated here rather than discovered by a
reader later:
...
class AxisComparison:
    ...
    def never_behind(self) -> bool:
        ...
        DISTINCT FROM `clear`, AND THE DISTINCTION IS THE WHOLE POINT. A
        challenger that ties the baseline in four runs and beats it in one has
        not beaten it in every run, so the survival claim fails -- but calling
        that STRADDLES would say the sample cannot order them, when in fact it
        ordered them and found no run on the losing side.
        """
        if self.tally_valid:
