...
wrote. The hash embedder is a bag of tokens. The relational leg computes its
...
THE OFFLINE PATH REMAINS THE DEFAULT AND STAYS DEPENDENCY-FREE.
`pip install -r requirements.txt && pytest` installs nothing from this file's
imports and never touches a network. Every driver import happens inside a
constructor, behind an injected-client escape hatch, and a missing driver
produces a sentence naming the package rather than a traceback -- the same
shape as router/providers.py, and for the same reason: a sibling repository
shipped provider tests that constructed live clients, passed on the
maintainer's machine, and failed on all three Python versions in CI.
...
`ElasticsearchBackend` therefore takes an `analyzer` argument with two settings
-- one mirroring this repository's own tokenizer, one Elasticsearch's default
English analysis -- so the tokenizer's contribution can be measured instead of
assumed. The scoring constants are already identical: BM25 defaults to
k1=1.2, b=0.75 in both.
"""
...
# "a real BM25 scores differently" from "a real analyzer tokenizes differently"
# -- two changes that arrive together when you simply swap the store.
MATCHING_ANALYZER = {
    "settings": {
        "analysis": {
            "filter": {
            ...
            "tokenizer": {
                "frr_pattern": {
                    ...
                    # the end -- the defect that made `queue.prefetch.count`
                    # unfindable when it appeared at the end of a sentence.
                    "pattern": _TOKEN.pattern,
                    "group": 0,
                    # CASE_INSENSITIVE IS LOAD-BEARING AND THE PATTERN IS WRONG
                    ...
                    # DELIMITER: "Checkout Service" tokenized to
                    # ["heckout", "ervice"] and "ERR_UPSTREAM_4423" to
                    # ["_", "_4423"]. Same regex source, opposite pipeline
                    # order, silently different tokens.
...
class DuckDBBackend:
    ...
    if that holds it is a result rather than an anticlimax. The relational leg
