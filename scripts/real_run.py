...
The rest of the repository runs on a hash embedder and a keyword router,
offline, and the gate never consults a provider. This script is the
...
def section_repeat(corpus, providers, first) -> None:
    ...
    The memo in CachingEmbedder embeds a text once, so asking the SAME cached
    embedder again would measure a dict. This builds a second embedder over
    the same model and pays for the corpus a second time. The cheapest
    section of the run, and the only one that can say whether the numbers
    above reproduce at all.
    """
    print(RULE)
